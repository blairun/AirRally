...
class AudioManager(context: Context) {
    ...
    init {
        ...
        // Rally bonus sounds
        soundMap[SoundEvent.SPIN_INFINITE] = soundPool.load(context, R.raw.spin_infinite, 1)
        soundMap[SoundEvent.COPY_CAT_MEOW] = soundPool.load(context, R.raw.copy_cat_meow, 1)
        soundMap[SoundEvent.COPY_CAT_MISS] = soundPool.load(context, R.raw.copy_cat_miss, 1)
        soundMap[SoundEvent.EXPLOSION] = soundPool.load(context, R.raw.explosion, 1)
        soundMap[SoundEvent.BANANA_SLIP] = soundPool.load(context, R.raw.banana_slip, 1)
        soundMap[SoundEvent.SHIELD_BREAK] = soundPool.load(context, R.raw.shield_block_break, 1)
        soundMap[SoundEvent.POWER_OUTAGE] = soundPool.load(context, R.raw.power_outage, 1)
        soundMap[SoundEvent.POWER_BACK_ON] = soundPool.load(context, R.raw.power_back_on, 1)
    ...
    private fun playDebugTone(event: SoundEvent) {
        try {
            when (event) {
                ...
                // Rally bonus sounds
                SoundEvent.SPIN_INFINITE -> toneGenerator.startTone(android.media.ToneGenerator.TONE_CDMA_ALERT_NETWORK_LITE, 500)
                SoundEvent.COPY_CAT_MEOW -> toneGenerator.startTone(android.media.ToneGenerator.TONE_CDMA_ANSWER, 300)
                SoundEvent.COPY_CAT_MISS -> toneGenerator.startTone(android.media.ToneGenerator.TONE_SUP_ERROR, 200)
                SoundEvent.EXPLOSION -> toneGenerator.startTone(android.media.ToneGenerator.TONE_CDMA_EMERGENCY_RINGBACK, 500)
                SoundEvent.BANANA_SLIP -> toneGenerator.startTone(android.media.ToneGenerator.TONE_PROP_NACK, 200)
                SoundEvent.SHIELD_BREAK -> toneGenerator.startTone(android.media.ToneGenerator.TONE_CDMA_PIP, 150)
                SoundEvent.POWER_OUTAGE -> toneGenerator.startTone(android.media.ToneGenerator.TONE_SUP_ERROR, 500)
                SoundEvent.POWER_BACK_ON -> toneGenerator.startTone(android.media.ToneGenerator.TONE_PROP_ACK, 300)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
