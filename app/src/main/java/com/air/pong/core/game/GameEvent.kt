...
sealed class GameEvent {
    data class YouServed(val swingType: SwingType, val spinType: SpinType = SpinType.NONE) : GameEvent()
    data class YouHit(val swingType: SwingType, val spinType: SpinType = SpinType.NONE) : GameEvent()
    data class OpponentHit(val swingType: SwingType, val spinType: SpinType = SpinType.NONE) : GameEvent()
    ...
    // Special Squares
    data class LandmineHit(val id: Long = System.nanoTime()) : GameEvent()
    data class BananaHit(val swingType: SwingType, val penalty: Int) : GameEvent()
