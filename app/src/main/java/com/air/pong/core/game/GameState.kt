...
data class GameState(
    ...
    val rallyCellTiers: List<Int> = List(9) { 0 }, // Tier index per cell (0=1pt, 1=3pt, 2=8pt...)
    val highestTierAchieved: Int = 0, // Highest tier this player has achieved (for extra life)
    val rallySoundEvent: RallySoundEvent? = null, // Sound event to play (consumed by ViewModel)
    val lastHitGridIndex: Int = -1, // Last grid cell hit (-1 for serve/none)
    val isServeHit: Boolean = false, // Was the last hit a serve (points without grid mark)
    ...
    // Cumulative Lines Cleared (for end game display)
...
fun GameState.calculateHitWindow(settings: SwingSettings): Long {
    ...
    // Rally Mode: Tier-based shrink (instead of per-shot shrink)
    // Shrinks 50ms for each tier achieved globally (max of both players)
    if (gameMode == GameMode.RALLY && isRallyShrinkEnabled) {
        val globalHighestTier = maxOf(highestTierAchieved, opponentHighestTier)
        val tierShrink = globalHighestTier * GameEngine.RALLY_TIER_SHRINK_MS
