...
class GameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        // Observe Rally Sound Events
        viewModelScope.launch {
            gameState.collect { state ->
                state.rallySoundEvent?.let { soundEvent ->
                    when (soundEvent) {
    ...
    fun checkRallyHighScore(): Boolean {
        val state = gameEngine.gameState.value
        val currentScore = state.rallyScore
        ...
        val linesThisGame = state.totalLinesCleared + state.partnerTotalLinesCleared
        ...
        val myHighestTier = state.highestTierAchieved
        val partnerHighestTier = state.opponentHighestTier
    ...
    private fun sendGameStateSync() {
        ...
        // Encode Tier Bitmasks
        val cellTiersBitmask = encodeTiersBitmask(state.rallyCellTiers)
        val opponentTiersBitmask = encodeTiersBitmask(state.opponentCellTiers)
        ...
        val msg = GameMessage.GameStateSync(
            ...
            highestTierAchieved = state.highestTierAchieved.toByte(),
            opponentHighestTier = state.opponentHighestTier.toByte()
        )
        sendMessage(msg)
    }
