...
class AccelerometerSensorProvider(context: Context) : SensorProvider {
    ...
    private var swingThreshold = GameEngine.DEFAULT_SOFT_THRESHOLD 
    ...
    // Peak gyro tracking during swing window
    private var peakGyroX = 0f
    private var peakGyroY = 0f
                ...
                if (it.sensor.type == Sensor.TYPE_LINEAR_ACCELERATION) {
                    ...
                    if (isCollectingPeak) {
                        ...
                        if (now - peakStartTime < PEAK_WINDOW_MS) {
                            ...
                            // Also track peak gyro independently
                            val currentGyroMag = sqrt(lastGyroX*lastGyroX + lastGyroY*lastGyroY + lastGyroZ*lastGyroZ)
                            if (currentGyroMag > peakGyroMagnitude) {
                                peakGyroMagnitude = currentGyroMag
                                peakGyroX = lastGyroX
                                peakGyroY = lastGyroY
                                peakGyroZ = lastGyroZ
                            }
                            ...
                            pendingPeakEvent = SwingEvent(peakStartTime, peakMagnitude, x, y, z, lastGyroX, lastGyroY, lastGyroZ, peakGyroX, peakGyroY, peakGyroZ, lastGravX, lastGravY, lastGravZ)
                        ...
                        if (magnitude > swingThreshold) {
                            if (now - lastSwingTime > DEBOUNCE_MS) {
                                ...
                                peakStartTime = now
                                peakMagnitude = magnitude
                                // Initialize peak gyro with current values
                                peakGyroX = lastGyroX
                                peakGyroY = lastGyroY
                                peakGyroZ = lastGyroZ
                                peakGyroMagnitude = sqrt(lastGyroX*lastGyroX + lastGyroY*lastGyroY + lastGyroZ*lastGyroZ)
                                // Use peakStartTime here as well. Gravity will be updated on emit.
                                pendingPeakEvent = SwingEvent(peakStartTime, magnitude, x, y, z, lastGyroX, lastGyroY, lastGyroZ, peakGyroX, peakGyroY, peakGyroZ, lastGravX, lastGravY, lastGravZ)
                            }
                        }
                    }
