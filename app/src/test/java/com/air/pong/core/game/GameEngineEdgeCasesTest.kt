...
class GameEngineEdgeCasesTest {
    ...
    fun `test swing ignored during cooldown`() {
        ...
        val result1 = gameEngine.processSwing(1000L, 25f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
        assertEquals(HitResult.HIT, result1)
        
        // Second swing at T=1100 (only 100ms later, should be ignored due to 500ms cooldown)
        val result2 = gameEngine.processSwing(1100L, 25f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
    ...
    fun `test swing allowed after cooldown expires`() {
        ...
        val result = gameEngine.processSwing(validHitTime, 25f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
    ...
    fun `test minimum hit window floor is enforced`() {
        disableRisk()
        gameEngine.updateSettings(1000L, 300, false, false, GameEngine.DEFAULT_SWING_THRESHOLD, true)
        ...
        repeat(50) { i ->
            if (i % 2 == 0) {
                // I hit
                gameEngine.processSwing(1000L + i * 1000, 10f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
            } else {
                // Opponent hits
    ...
    fun `test hit window at difficulty boundaries`() {
        ...
        // Test MIN difficulty
        gameEngine.updateSettings(1000L, GameEngine.MIN_DIFFICULTY, false, false, GameEngine.DEFAULT_SWING_THRESHOLD, false)
        assertEquals(GameEngine.MIN_DIFFICULTY.toLong(), gameEngine.getHitWindow())
        
        // Test MAX difficulty
        gameEngine.updateSettings(1000L, GameEngine.MAX_DIFFICULTY, false, false, GameEngine.DEFAULT_SWING_THRESHOLD, false)
    ...
    fun `test swing at exact threshold boundaries`() {
        disableRisk()
        val threshold = GameEngine.DEFAULT_SWING_THRESHOLD // 14.0f
        
        // Exactly at SOFT/MEDIUM boundary (threshold + 9 = 23)
        gameEngine.startGame()
        gameEngine.processSwing(1000L, 23.0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
        assertEquals(SwingType.SOFT_FLAT, gameEngine.gameState.value.lastSwingType)
        
        // Just over SOFT/MEDIUM boundary
        gameEngine.processSwing(2000L, 23.1f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
        ...
        gameEngine.processSwing(3000L, 44.0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
        assertEquals(SwingType.MEDIUM_FLAT, gameEngine.gameState.value.lastSwingType)
        
        // Just over MEDIUM/HARD boundary
        gameEngine.processSwing(4000L, 44.1f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
    ...
    fun `test swing type with extreme tilt values`() {
        ...
        gameEngine.startGame()
        gameEngine.processSwing(1000L, 25f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 10.0f)
        assertTrue(gameEngine.gameState.value.lastSwingType?.isLob() == true)
        
        // Extreme SMASH (very low negative Z)
        gameEngine.processSwing(2000L, 25f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, -10.0f)
    ...
    fun `test longest rally tracking`() {
        ...
        gameEngine.processSwing(arrivalTime - 100, 10f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
