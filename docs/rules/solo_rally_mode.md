...
Smash serves (snappier timing):
```
Timeline: 0% ──── 15% ──────── 50% ────────────── 80% ──── 100%
          │       │            │                   │         │
          Serve  Table       Wall                Table      Hit
          (hit)  bounce      bounce              bounce ←   window
                 (fast)                          (return)   opens
...
| Event | Standard | Smash | Sound |
|-------|----------|-------|-------|
| Player serves | 0% | 0% | `hit.wav` |
| Table bounce (outgoing) | 25% | 15% | `bounce.wav` |
| Wall bounce | 50% | 50% | `bounce_wall.wav` (50% volume) |
| Table bounce (return) | 75% | 80% | `bounce.wav` |
| Hit window opens | 75% | 80% | Screen → Green |
...
Solo Rally includes the same bonus mechanics as Co-op:

- **Spin Shots**: Varied spins increase your multiplier.
- **Copy Cat**: Follow the CPU-generated sequence to build your multiplier.
- **Special Squares**: Golden, Banana peel, and Landmine squares appear as you progress.

For full details, see [Rally Mode Bonuses](rally_mode_bonuses.md).
